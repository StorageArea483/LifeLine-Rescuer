...
class RequestsCardSheet {
	static void show(BuildContext context,
			{required int activeRequests, List<String>? assignmentIds}) {
		showModalBottomSheet(
			isDismissible: true,
			context: context,
			isScrollControlled: true,
			backgroundColor: Colors.transparent,
			builder: (context) => RequestSheet(
...
class RequestSheet extends ConsumerStatefulWidget {
	...
	const RequestSheet({super.key, required this.activeRequests, this.assignmentIds});
...
class _RequestSheetState extends ConsumerState<RequestSheet> {
	...
	Widget _buildRequestCard(Map<String, dynamic> victim, WidgetRef ref) {
		...
		return Container(
			margin: EdgeInsets.only(bottom: ResponsiveHelper.isTablet(context) ? 24 : 16),
			decoration: BoxDecoration(
				color: AppColors.surfaceLight,
				borderRadius: BorderRadius.circular(16),
				border: Border.all(color: AppColors.primaryMaroon.withOpacity(0.1), width: 1),
			),
			child: Container(
        ...
        children: [
          Expanded(
              ...
              style: ElevatedButton.styleFrom(
                ...
                padding: EdgeInsets.symmetric(
                  vertical:
                      ResponsiveHelper.isTablet(context) ? 18 : 14,
                ),
